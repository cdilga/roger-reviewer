...
| [`docs/SELF_EXPLORATION_AND_REVIEW_WORKBENCH_RECOVERY_PLAN.md`](docs/SELF_EXPLORATION_AND_REVIEW_WORKBENCH_RECOVERY_PLAN.md) | Active recovery plan for self-exploration, bridge-launched resume, TUI workbench completion, GitHub-context authoring, and final-submission memory capture |
| [`docs/ENVIRONMENT_ALIGNMENT_AND_NEXT_ITERATION_PLAN.md`](docs/ENVIRONMENT_ALIGNMENT_AND_NEXT_ITERATION_PLAN.md) | Historical launch-gate plan for local/devbox alignment; resolved 2026-07-06 by choosing local `main` as convergence authority (devbox is compute offload only) |
| [`docs/PRODUCT_SURFACE_RECOVERY_AND_RELEASE_PLAN.md`](docs/PRODUCT_SURFACE_RECOVERY_AND_RELEASE_PLAN.md) | Active umbrella plan: seven-verb CLI simplification, extension bootstrap/feedback/update-freshness, TUI completion, Copilot live drop-in, ticketed live proofs, and the next CalVer release gate |
