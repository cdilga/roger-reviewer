...
description: Use when running Roger reviews on the feature-gated GitHub Copilot CLI provider — enabling the gate, passing doctor preflight, understanding the review_readonly policy profile, and exercising Tier B continuity (reopen, rr return, reseed fallback).
---
...
- shell execution, write tools, external URL/network access
...
expected behavior, not an error to route around.
