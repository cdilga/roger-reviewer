---
name: Roger Operator Quickstart
description: Use for a first-session walkthrough of Roger Reviewer — install, doctor preflight, picking review work, starting a review, where findings actually come from, the local cockpit, findings/search, and the outbound send chain. Truthful about which provider paths are live vs feature-gated.
---

# Roger Operator Quickstart

This is the first-session tour of `rr`. For the full operator loop and robot
envelopes, see `roger-review-driver`. For the feature-gated Copilot lane, see
`roger-copilot-harness`. For the worker-side transport used inside a provider
session, see `roger-worker-protocol`. For cockpit keybindings, see
...
that session through the `rr agent` transport (see `roger-worker-protocol`
...
command paths the shell uses. See `roger-tui-cheatsheet` for the exact screens and keys.
...
- Feature-gated, not default: the GitHub Copilot CLI provider — needs
  `RR_ENABLE_COPILOT_PROVIDER=1`. See `roger-copilot-harness`.
- `rr agent` is a separate in-session transport for the review worker, not
  ...
  operator — see `roger-worker-protocol` if you end up inside a session.
