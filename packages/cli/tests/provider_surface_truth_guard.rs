...
fn provider_support_truth_guard_matches_live_cli_help_and_docs() {
    ...
    assert_contains_all(
        ...
        &[
            ...
            "**OpenCode** is the first-class provider and the recommended default",
            "**Codex, Gemini, and Claude Code** are bounded Tier A providers.",
            ...
            "reopen by locator or session id, return\n  with `rr return`, and fall back to honest `ResumeBundle` reseed",
        ],
    ...
    assert_contains_all(
        &agents,
        &[
            "Every Roger review session must map to",
            ...
            "| GitHub Copilot CLI | Feature-gated opt-in continuity provider | No (gated, off by default) | Bounded Tier B when enabled |",
            "GitHub Copilot CLI is a feature-gated opt-in, disabled by default; do not",
            "describe it as preferred or default.",
