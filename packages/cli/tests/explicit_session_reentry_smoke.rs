...
#[test]
fn resume_explicit_session_blocks_cross_root_binding_reuse() {
    ...
    let resume = run_rr(
        &["resume", "--session", session_id.as_str(), "--robot"],
        &resume_runtime,
    );
...
#[test]
fn return_explicit_session_blocks_cross_root_binding_reuse() {
    let temp = tempdir().expect("tempdir");
    let repo_a = init_repo_named(&temp, "repo-a");
    let repo_b = init_repo_named(&temp, "repo-b");
    let (_stub_dir, opencode_bin) = write_stub_binary();
    ...
    let ret = run_rr(
        &["return", "--session", session_id.as_str(), "--robot"],
        &return_runtime,
    );
    ...
    assert_eq!(
        count_launch_attempts(&store_root, "return_to_roger", &session_id),
        0
    );
}
