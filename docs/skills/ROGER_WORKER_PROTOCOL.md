...
Purpose:
Canonical recipe for being a Roger review worker inside a provider session:
binding to a `ReviewTask` via a task file, the exact `rr agent worker.*` call
sequence, the `WorkerStageResult`/`StructuredFindingsPack` return shapes, and
the hard boundaries a worker must never cross.
...
`ROGER_REVIEW_DRIVER.md` instead. If you are in a bare-harness Roger-native
subset without a bound `rr agent` task, use `ROGER_INSIDE_ROGER_AGENT.md`
