...
Purpose:
Drive a pull request review with the rr CLI from outside a provider
session: start, resume, inspect, draft, edit, approve, and post through
Roger's explicit gates, using robot envelopes and fail-closed recovery.
...
and need to call the worker transport, use `ROGER_WORKER_PROTOCOL.md`
instead. If you are in a bare-harness Roger-native command subset without a
bound `rr agent` task, use `ROGER_INSIDE_ROGER_AGENT.md` instead.
