...
| `INV-POST-002` | Target drift, rebase, rerun, or anchor drift invalidates prior approval automatically | `unit` + `integration` | `prop_*`, `int_github_*` | `fixture_refresh_rebase_target_drift`, `fixture_github_draft_batch` | invalidation summary, refreshed draft snapshot, approval-state transition evidence | `rr-011.2`, `rr-011.4`, `rr-020`, `rr-x51h.8.4` |
| `INV-POST-003` | Posting from the wrong repo, wrong PR, or stale session binding fails closed and preserves a local repair path | `integration` | `int_github_*` | `fixture_partial_post_recovery`, target-drift fixtures | adapter response snapshot, posted-action or failure snapshot, retry-path evidence | `rr-020`, `rr-011.4`, `rr-x51h.8.4` |
| `INV-POST-004` | CLI status/findings and TUI queue/inspector project the same outbound state machine (`awaiting_approval`, `approved`, `posted`, `invalidated`, `failed`) and keep posting readiness visibly elevated | `integration` | `int_cli_*`, `int_tui_*` | `fixture_github_draft_batch`, `fixture_partial_post_recovery`, target-drift fixtures | robot status/findings envelopes, TUI controller-state snapshot, posting-gate visibility evidence | `rr-ph77.2`, `rr-ph77.2.1`, `rr-ph77.2.2`, `rr-ph77.6` |
...
| `INV-SESSION-002` | `resume`, `refresh`, and `return` operate only on valid resolvable session identity or fail closed truthfully | `integration` | `int_cli_*`, `accept_opencode_*`, `accept_codex_*`, `accept_gemini_*`, `accept_claude_*`, `accept_copilot_*` | `fixture_resumebundle_stale_locator`, `fixture_opencode_dropout_return` | resume bundle snapshot, session locator evidence, continuity result summary | `rr-015`, `rr-011.5`, `rr-018`, `rr-x51h.8.3` |
...
| `INV-PROVIDER-001` | Provider capability surfaces expose truthful policy profile ids and digests, hook/instruction contract versions, audit artifact classes, MCP posture, and worktree-root-aware routine surface data instead of relying on prose-only support claims | `integration` | `int_cli_*`, `accept_codex_*`, `accept_gemini_*`, `accept_claude_*`, `accept_copilot_*` | `fixture_resumebundle_stale_locator`, `fixture_copilot_launch_resume`, `fixture_copilot_hook_stream`, `fixture_copilot_policy_failure`, `fixture_copilot_crash_recovery` | robot doctor/status/findings/review/resume envelopes, provider-support guide snapshot, provider launch-capture or hook-artifact references where relevant | `rr-x51h.8.5`, `rr-x51h.8.3`, `rr-8isd.5.2` |
...
| `INV-UPDATE-004` | A representative published-to-published upgrade rehearsal proves install `N`, run old `rr`, update to `N+1`, and retain a usable binary with truthful post-update state | `integration` + `smoke` | `int_cli_update_upgrade_*`, `smoke_update_upgrade_*` | synthetic prior-release bundles, isolated install dirs, prior-schema fixtures when required | upgrade rehearsal manifest, pre/post version summary, post-update smoke output, retained failure artifacts on interruption | `rr-5urd.1`, `rr-1xhg.5`, future release/update validation beads |
